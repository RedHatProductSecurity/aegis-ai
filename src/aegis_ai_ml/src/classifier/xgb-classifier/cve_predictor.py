...
import argparse
import re
from cve_data_scraper import CVEDataScraper
from cve_feature_extraction import CVEFeatureExtractor
...
class CVEPredictor:
    ...
    def check_repository_setup(self):
        ...
        if not source_vulns_repo.exists():
            missing_repos.append(f"Security vulnerabilities repo: {source_vulns_repo.resolve()}")
        elif not (source_vulns_repo / ".git").exists():
            logger.warning(f"Security repo exists but is not a git repository: {source_vulns_repo.resolve()}")
        ...
        if not source_linux_repo.exists():
            ...
            logger.warning(f"Linux repo exists but is not a git repository: {source_linux_repo.resolve()}")
    ...
    def setup_predict_environment(self):
        ...
        if not self.check_repository_setup():
            raise FileNotFoundError("Required repositories are missing. Please run the data scraper first.")
        ...
        # Create symlinks
        for link, source in [(predict_vulns_repo_link, source_vulns_repo), 
            ...
            try:
                ...
                if not source.exists():
                    ...
                    raise FileNotFoundError(f"Source repository not found: {source.resolve()}")
    ...
    def _ensure_commit_exists(self, commit_hash: str, repo_path: Path):
        ...
        result = subprocess.run(check_cmd, cwd=repo_path, capture_output=True, text=True)
        ...
        try:
            ...
            subprocess.run(fetch_cmd, cwd=repo_path, capture_output=True, text=True, timeout=30)
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
            # If specific fetch fails, trying a broader fetch
            try:
                fetch_cmd = ["git", "fetch", "origin", "--depth=100"]
                subprocess.run(fetch_cmd, cwd=repo_path, capture_output=True, text=True, timeout=60)
            except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
    ...
    def scrape_from_commits(self) -> bool:
        ...
        start_time = time.time()
        logger.info(f"Fetching commits directly for {self.cve_id}: {[h[:8] + '...' for h in self.commit_hashes]}")
        ...
        for commit_hash in self.commit_hashes:
            try:
                ...
                result = subprocess.run(show_cmd, cwd=linux_repo_path, 
                                      capture_output=True, text=True, check=True)
                commit_info['commit_details'] = result.stdout
                
                # Get the actual patch
                patch_cmd = ["git", "show", commit_hash]
                result = subprocess.run(patch_cmd, cwd=linux_repo_path,
                                      capture_output=True, text=True, check=True)
                commit_info['patch_content'] = result.stdout
                
                # Get list of changed files
                files_cmd = ["git", "show", "--name-only", "--format=", commit_hash]
                result = subprocess.run(files_cmd, cwd=linux_repo_path,
                                      capture_output=True, text=True, check=True)
                commit_info['changed_files'] = [f.strip() for f in result.stdout.split('\n') if f.strip()]
                
                # Get diff stats
                stats_cmd = ["git", "show", "--stat", "--format=", commit_hash]
                result = subprocess.run(stats_cmd, cwd=linux_repo_path,
                                      capture_output=True, text=True, check=True)
        ...
        metadata = {
            ...
            'commit_urls': [f"https://git.kernel.org/stable/c/{commit}" for commit in found_commits],
        ...
        if success:
            logger.info(f"Successfully scraped {len(found_commits)}/{len(self.commit_hashes)} commits ({elapsed_time:.2f}s)")
        else:
    ...
    def extract_features(self) -> Optional[Dict]:
        ...
        try:
            ...
            features_result = engineer.process_cve_with_timeout(self.cve_id, timeout_seconds=60)
            ...
            if features_result:
                ...
                clean_features = {k: v for k, v in features_result.items() if k in self.feature_columns}
                return clean_features
            else:
                logger.error(f"Failed to extract features for {self.cve_id}! ({elapsed_time:.2f}s)")
                return None
        except Exception as e:
    ...
    def predict_severity(self) -> Optional[Dict]:
        ...
        result = {
            ...
            'active_features': [
                feature for feature in self.feature_columns 
                if features.get(feature, False)
            ],
            'input_method': 'direct_commits' if self.commit_hashes else 'traditional_scraping',
    ...
    def predict(self) -> Optional[Dict]:
        ...
        if self.commit_hashes:
            logger.info(f"Using direct commit mode with {len(self.commit_hashes)} commits")
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"Prediction for {self.cve_id} completed in {total_elapsed_time:.2f}s")
            return result
        except Exception as e:
            logger.error(f"Unexpected error during prediction pipeline for {self.cve_id}: {e}")
            return None
    ...
    def print_result(self, result: Optional[Dict]):
        ...
        if result.get('commit_hashes'):
            print(f"Commit Hashes: {', '.join([h[:8] + '...' for h in result['commit_hashes']])}")
...
def main():
    ...
    parser.add_argument('--commit', type=str, help='Single commit hash to analyze directly')
    parser.add_argument('--commits', type=str, help='Comma-separated list of commit hashes')
