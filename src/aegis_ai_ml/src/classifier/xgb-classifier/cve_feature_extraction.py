...
class CVEFeatureExtractor:
    def __init__(self, data_dir: str = "data"):
        ...
        self.feature_names = [
            'kasan', 'lock', 'write', 'read', 'fixprintf', 'remote', 'danger', 'bpf',
            'nullptr', 'disk', 'init', 'warnonly', 'outofbounds', 'deadlock', 'trace',
            'usb', 'nomemchk', 'virt', 'simplefix', 'netfilter', 'nvme', 'race',
            'leak', 'errorpath', 'uaf', 'networking', 'notincludedcomponent', 'skb',
            'dma', 'logger', 'improve', 'kernel_panic_plus_uaf', 'timer', 'spectre',
            'hardware', 'servertoclientfail', 'tipc', 'debugfs', 'linus', 'cveoutdated',
            'boot', 'test', 'qdisc', 'ipv6', 'iouring', 'syzbot', 'kernel_panic', 'memory'
        ]
    ...
    def clean_content(self, content: str) -> str:
        ...
        # limiting size for mem 
        if len(content) > 10 * 1024 * 1024:  # 10MB limit
            logger.warning(f"Truncating large content: {len(content)} bytes")
    ...
    def extract_patch_features(self, patch_content: str, patch_filename: str = "") -> Dict[str, bool]:
        ...
        try:
            ...
            if len(patch_content) > 5 * 1024 * 1024:  # 5MB limit per patch
                logger.warning(f"Large patch content: {len(patch_content)} bytes, truncating")
            ...
            commit_msg_match = re.search(r'(.*?)^diff --git', patch_content, re.MULTILINE | re.DOTALL)
            if commit_msg_match:
                commit_msg = commit_msg_match.group(1)
                lines_text = len([l for l in commit_msg.split('\n') if l.strip()])
            ...
            search_content = patch_content[:100000] if len(patch_content) > 100000 else patch_content
            ...
            if len(uniq_lines) > 5000:
                logger.warning(f"Large unique line set: {len(uniq_lines)}, sampling first 5000")
    ...
    def _analyze_file_path(self, file_path: str) -> Dict[str, bool]:
        ...
        remote_patterns = [
            ...
            r'include/net', r'drivers/block/nbd', r'fs/smb', r'fs/netfs', r'fs/jfs'
        ]
        features['remote'] = any(re.search(pattern, file_path) for pattern in remote_patterns)
        ...
        # Hardware drivers
        hardware_patterns = [r'drivers/gpu', r'sound/pci', r'sound/soc', r'kernel/events', r'drivers/media']
        features['hardware'] = any(re.search(pattern, file_path) for pattern in hardware_patterns)
        ...
        networking_patterns = [
            r'drivers/net', r'net/bluetooth', r'net/ipv4', r'net/ipv6', 
            r'net/sctp', r'gsm', r'net/unix', r'net/sched'
        ]
        features['networking'] = any(re.search(pattern, file_path) for pattern in networking_patterns)
        ...
        excluded_patterns = [r'fs/afs', r'btrfs', r'lib/kunit', r'lib/.*kunit', r'arch/riscv', r'lib/test']
        features['notincludedcomponent'] = any(re.search(pattern, file_path) for pattern in excluded_patterns)
        
        # Disk/filesystem
        disk_patterns = [
            r'block/ioctl', r'fs/(btrfs|pstore|ext4|fat|fscache|hugetlbfs|iomap|kernfs|lockd|minix|nfs|nfs_common|nfsd|overlayfs|proc|quota|ramfs|sysfs|xfs)',
            r'drivers/md', r'fs/'
        ]
        features['disk'] = any(re.search(pattern, file_path) for pattern in disk_patterns)
    ...
    def _analyze_patch_content(self, content: str) -> Dict[str, bool]:
        ...
        features['kasan'] = any(re.search(pattern, content_lower) for pattern in kasan_patterns)
        
        # Linus Torvalds and other key developers
        linus_patterns = [
            r'linus torvalds', r'eric dumazet', r'or cohen', r'sagi grimberg', r'keith busch',
            r'yi yang', r'greg kroah-hartman', r'david ahern', r'paolo abeni'
        ]
        features['linus'] = any(re.search(pattern, content_lower) for pattern in linus_patterns)
        
        # Out of bounds
        oob_patterns = [
            r'address\s', r'buffer overflow', r'underflow', r'buffer-overflow', r'oob',
            r'out-of-bound', r'buffer boundar', r'out of bound', r'out-of-range',
            r'out of range', r'stack overflow', r'stack-overflow', r'bounds check'
        ]
        features['outofbounds'] = any(re.search(pattern, content_lower) for pattern in oob_patterns)
        ...
        # Error path
        features['errorpath'] = bool(re.search(r'error path|error handling', content_lower))
        
        # Read operations
        read_patterns = [
            r'uninit.value in.*?\+', r'uninitialized value', r'slab.out.of.bounds read',
            r'read of size \d+', r'reads from', r'read from', r'mmap', r'read path',
            r'read-after-free', r'read after free'
        ]
        features['read'] = any(re.search(pattern, content_lower) for pattern in read_patterns)
        
        # Write operations
        write_patterns = [
            r'write to.*?of \d+ bytes', r'write\(\)', r'write_buf', r'free-write',
            r'free write', r'concurrent write', r'oob write', r'oob-write', r'oobw',
            r'buffer overrun', r'read\swrite', r'stack.out.of.bounds'
        ]
        features['write'] = any(re.search(pattern, content_lower) for pattern in write_patterns)
        
        # Adjust read/write based on priority
        if features.get('write'):
            features['read'] = False
        if re.search(r'uninit.value in.*?\+|slab.out.of.bounds read|read of size \d+|read-after-free|read after free', content_lower):
            features['read'] = True
        ...
        # Memory leaks
        leak_patterns = [r'leaked', r'memory leak', r'leak', r'that can grow pretty large', r'low-memory', r'low memory']
        features['leak'] = any(re.search(pattern, content_lower) for pattern in leak_patterns)
        ...
        # Initialization
        init_patterns = [r'setup.*init', r'_init', r'initialization', r'qdisc_create', r'_shutdown']
        features['init'] = any(re.search(pattern, content_lower) for pattern in init_patterns)
        
        # NULL pointer dereference
        nullptr_patterns = [
            r'null pointer deref', r'null-pointer-deref', r'nullptr', r'null ptr',
            r'null-ptr', r'null dereference', r'null-dereference'
        ]
        features['nullptr'] = any(re.search(pattern, content_lower) for pattern in nullptr_patterns)
        
        # Warning only
        warn_patterns = [r'warning: cpu', r'warning:', r'warn/', r'blocked for more than', r'warn\b', r'warn on', r'mismatch']
        features['warnonly'] = any(re.search(pattern, content_lower) for pattern in warn_patterns)
        ...
        # Use-after-free
        uaf_patterns = [
            r'double-free', r'invalid-free', r'use-after-free', r'use after free',
            r'uaf', r'freed\s', r'divide-by-zero', r'divide by zero\W'
        ]
        features['uaf'] = any(re.search(pattern, content_lower) for pattern in uaf_patterns)
        if features['uaf']:
            features['kasan'] = False
        
        # Internal error
        if re.search(r'internal error: oops:', content_lower) and not features.get('read'):
            features['kasan'] = False
        ...
        if (re.search(r'code:', content_lower) and re.search(r'rip:', content_lower) and features.get('uaf')):
        ...
        # Danger indicators
        danger_patterns = [
            r'double free', r'to userspace', r'syzbot', r'crash', r'panic',
            r'kernel bug', r'corruption', r'rce', r'lpe', r'bug:', r'dangling pointer', r'_state'
        ]
        features['danger'] = any(re.search(pattern, content_lower) for pattern in danger_patterns)
        
        # Reduce danger if other indicators present
        if (features['danger'] and re.search(r'bug:', content_lower) and 
            (features.get('kasan') or features.get('warnonly') or features.get('leak') or 
             features.get('fixprintf') or features.get('bpf') or features.get('trace') or features.get('init'))):
        ...
        if (any(kernel_panic_conditions) and not features.get('read') and 
            not re.search(r'potential', content_lower) and not features.get('errorpath')):
        ...
        if features.get('kernel_panic'):
            ...
            if features.get('kernel_panic_plus_uaf'):
                features['kernel_panic'] = True
            if ((features.get('outofbounds') and (features.get('ipv6') or features.get('networking') or 
                 features.get('hardware') or features.get('disk')))):
                features['kernel_panic'] = True
        ...
        # Hardware detection
        hw_patterns = [r'_cpu_', r'device', r'amdgpu', r'hardware', r'alsa', r'(\s|\-|_)arch', r'cpu timer/arm64', r'cpu']
        features['hardware'] = any(re.search(pattern, content_lower) for pattern in hw_patterns)
        
        # Deadlock
        features['deadlock'] = bool(re.search(r'deadlock|dead lock|dead-lock|lockup', content_lower))
        
        # Improve only
        features['improve'] = bool(re.search(r'proper|check\s|checks\s|improv', content_lower))
    ...
    def _analyze_code_lines(self, uniq_lines: set) -> Dict[str, bool]:
        ...
        for line in uniq_lines:
            ...
            # Locking
            lock_patterns = [
                r'mutex_lock', r'mutex_unlock', r'read_lock', r'read_unlock',
                r'spin_lock', r'spin_unlock', r'arch_spin_unlock', r'arch_spin_lock'
            ]
            ...
            # Init patterns
            init_patterns = [r'exit', r'init', r'register', r'flush', r'shutdown', r'disconnect', r'_free', r'_abort', r'_clear']
    ...
    def _analyze_author(self, patch_content: str) -> Dict[str, bool]:
        ...
        linus_patterns = [
            # Original names
            r'linus torvalds', r'eric dumazet', r'or cohen', r'sagi grimberg',
            r'keith busch', r'yi yang', r'greg kroah-hartman', r'qiumiao zhang',
            r'david ahern', r'paolo abeni', r'zack rusin',r'ilya dryomov', 
            r'kuniyuki iwashima', r'valis', r'jamal hadi salim',r'hangyu hua', 
            r't\.feng', r'david s\. miller', r'zhiqiang liu',r'feilong lin', 
            r'jens axboe', r'anthony steinhauser', r'thomas gleixner', r'luiz augusto von dentz', 
            r'marcel holtmann', r'thadeu lima de souza cascardo', r'michal luczaj', 
            r'sasha levin', r'subbaraya sundeep', r'tetsuo handa',
            r'florian westphal', r'pablo neira ayuso'
]
        
        # Check for less prominent developers (anti-Linus)
        anti_linus_patterns = [
            r'bailey forrest', r'willem de bruijn', r'catherine sullivan',
            r'christoph paasch', r'andy shevchenko'
        ]
    ...
    def process_cve_data_with_diagnostics(self, cve_id: str) -> Optional[Dict]:
        ...
        if metadata_file.exists():
            ...
            logger.info(f"  {cve_id}: No metadata file found, will process available data")
        ...
        if commits_dir.exists():
            ...
            for patch_file in patch_files:
                try:
                    ...
                    if file_size > 10 * 1024 * 1024:  # 10MB
                        logger.warning(f"  {cve_id}: Large patch file {patch_file.name}: {file_size / (1024*1024):.1f}MB")
                    ...
                    patch_features = self.extract_patch_features(patch_content, str(patch_file))
                
                    patch_time = time.time() - patch_start
                    if patch_time > 30:  # More than 30 seconds for one patch
                        logger.warning(f"  {cve_id}: Slow patch {patch_file.name}: {patch_time:.1f}s, size: {len(patch_content)} chars")
            ...
            if total_size > 1024*1024:
                logger.info(f"  {cve_id}: Total data size: {total_size / (1024*1024):.1f}MB")
    ...
    def process_cve_with_timeout(self, cve_id: str, timeout_seconds: int = 600) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"TIMEOUT: {cve_id} took longer than {timeout_seconds} seconds - SKIPPING")
            signal.alarm(0)
            return None
        except Exception as e:
    ...
    def create_dataset(self, output_file: str = "data/cve_dataset.csv") -> pd.DataFrame:
        ...
        for i, cve_id in enumerate(cve_ids, start_index + 1):
            logger.info(f"Processing {i}/{len(self.ground_truth_labels)}: {cve_id}")
        
            result = self.process_cve_with_timeout(cve_id, timeout_seconds=600)
            if result:
            ...
            # Save checkpoint every 100 CVEs (adjust numbering)
            if (i - start_index) % 100 == 0:
                checkpoint_df = pd.DataFrame(all_results)
                checkpoint_file = f"data/cve_checkpoint_{i}.csv"
                checkpoint_df.to_csv(checkpoint_file, index=False)
                logger.info(f"CHECKPOINT: Saved {len(all_results)} CVEs to {checkpoint_file}")

        logger.info(f"Completed processing. Successful: {len(all_results)}, Skipped: {len(skipped_cves)}")
