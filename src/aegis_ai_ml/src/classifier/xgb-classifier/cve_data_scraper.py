...
class CVEDataScraper:
    ...
    def _extract_cve_list(self) -> List[str]:
        ...
        for entry in cve_data:
            if 'cve_id' in entry and entry['cve_id'] is not None and entry['cve_id'].strip():
    ...
    def setup_linux_repo(self):
        ...
        if not self.linux_repo_path.exists():
            ...
            try:
                ...
                logger.warning(f"Git pull failed for Linux repo: {e}. Continuing with existing repo...")
    ...
    def setup_vulns_repo(self):
        ...
        if not self.vulns_repo_path.exists():
            ...
            logger.info("Security vulnerabilities repo exists. Updating with git pull...")
            try:
                ...
                logger.warning(f"Git pull failed for vulns repo: {e}. Continuing with existing repo...")
    ...
    def process_cve_from_vulns_repo(self, cve_id: str) -> Optional[Dict]:
        ...
        try:
            search_result = subprocess.run(
                ["find", str(self.vulns_repo_path), "-name", f"*{cve_id}*", "-type", "f"],
    ...
    def _extract_info_from_json(self, cve_data: Dict):
        ...
        fields_to_search = ['description', 'references', 'problemDescription', 'mitigation']
    ...
    def fetch_commit_info_from_linux_repo(self, cve_id: str, commit_urls: List[str]):
        ...
        logger.info(f"Commit summary for {cve_id}: {len(found_commits)} found, {len(missing_commits)} missing")
    ...
    def _ensure_commit_exists(self, commit_hash: str):
        ...
        result = subprocess.run(check_cmd, cwd=self.linux_repo_path, 
                              capture_output=True, text=True)
        
        if result.returncode == 0:
            return  # Commit exists
        
        # Try to fetch all refs to get more commits
        logger.info(f"Attempting to fetch commit {commit_hash} from remote")
        try:
            # Fetch all refs from origin
            fetch_cmd = ["git", "fetch", "origin", "--depth=50"]
            subprocess.run(fetch_cmd, cwd=self.linux_repo_path, 
                         capture_output=True, text=True, timeout=30)
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
            # If that fails, try a broader fetch
            try:
    ...
    def find_cve_commits_in_linux_repo(self, cve_id: str):
        ...
        try:
            ...
            git_log_cmd = [
                "git", "log", "--all", "--grep", cve_id, 
                "--pretty=format:%H", "--max-count=10"
            ]
            
            result = subprocess.run(git_log_cmd, cwd=self.linux_repo_path,
                                  capture_output=True, text=True, check=True)
            
            if result.stdout.strip():
                commit_hashes = result.stdout.strip().split('\n')
                commit_urls = [f"https://git.kernel.org/stable/c/{h}" for h in commit_hashes]
    ...
    def process_all_cves(self, limit: Optional[int] = None):
        ...
        logger.info(f"Completed processing. Success: {success_count}/{len(cves_to_process)}")
        ...
        for cve_id in self.cve_list:
            ...
            if metadata_file.exists():
                ...
                if commits_dir.exists():
                    ...
                    actual_found = [f for f in found_files if not f.name.endswith('_missing.json')]
    ...
    def export_summary_report(self, output_file: str = "cve_analysis_report.json"):
        ...
        for cve_id in self.cve_list:
            ...
            if metadata_file.exists():
                ...
                if commits_dir.exists():
                    # Get found commits
                    found_files = [f for f in commits_dir.glob("*.json") 
...
def main():
    ...
    logger.info(f"Missing commits analysis: {missing_analysis['total_missing']} total missing from {missing_analysis['cves_with_missing']} CVEs")
    
    # Export comprehensive report
    report = scraper.export_summary_report()
    
    # Print some insights
    if stats['processed_cves'] > 0:
        success_rate = (stats['found_commits'] / max(stats['total_commits'], 1)) * 100
        logger.info(f"Commit success rate: {success_rate:.1f}% ({stats['found_commits']}/{stats['total_commits']})")
        ...
        if missing_analysis['total_missing'] > 0:
            ...
            for cve_id, missing_commits in missing_analysis['missing_by_cve'].items():
                ...
                for commit in missing_commits[:2]:  # Show first 2 missing commits per CVE
                    logger.info(f"    - {commit['commit_hash']} ({commit['commit_url']})")
                count += 1
