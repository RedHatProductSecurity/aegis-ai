...
def sync_vulns_repo(
    ...
    *,
    ...
    This function is not safe to run concurrently against the same
    ``repo_path``. Callers must serialize access themselves: git does not
    support concurrent clone or pull operations on the same working tree, and
    concurrent access can corrupt the clone, especially when
    ``XDG_CONFIG_HOME`` points at a shared directory.
