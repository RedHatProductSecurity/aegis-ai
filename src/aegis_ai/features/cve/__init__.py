...
class SuggestImpact(Feature):
    ...
    async def exec(self, cve_id: CVEID, static_context: Any = None):
        prompt = AegisPrompt(
            ...
            """,
            rules="""
                - If any reference urls start with https://github.com always use github mcp tool to retrieve additional context from vulnerability reference url.
                - For all other reference urls use tavily tool to retrieve additional context.
...
class SuggestCWE(Feature):
    ...
    async def exec(self, cve_id: CVEID, static_context: Any = None):
        prompt = AegisPrompt(
            ...
            """,
            rules="""
                - Use mitre cwe tool to retrieve allowed cwe-ids first so this context can be used to try and identify cwe in vulnerability.
                - If any reference urls start with https://github.com always use github mcp tool to retrieve additional context from vulnerability reference url.
                - For all other reference urls use tavily tool to retrieve additional context.
                - Always use kernel_cve tool to provide additional CVE context if CVE component is kernel.
                - Use mitre search cwe to query for candidate CWEs and analyse with vulnerability data if applicable. 
                - If candidate CWE are below .65 score then reduce confidence.
                Output should include:
                - cwe: a list of top 1–3 most likely CWE IDs (e.g., ["CWE-94","CWE-22"]).
