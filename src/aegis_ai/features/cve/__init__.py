...
# Updated by A.Larkin (on 9 Sept 2026), dirty hack:
...
# End of "updated by A.Larkin (on 9 Sept 2026), dirty hack".
...
_ACTIONABLE_SCORE_PROMPT_PATH = (
    Path(__file__).with_name("actionable_score_prompt.txt")
)
...
async def _ensure_kernel_context_for_second_opinion(
    ...
    try:
        ...
        # -------------------------------------------------------------
        tool_func = (
            getattr(kernel_cve_tool, "function", None)
            or getattr(kernel_cve_tool, "func", None)
        )

        if tool_func is None:
            raise RuntimeError(
                "kernel_cve_tool underlying callable not found"
            )
...
def _parse_afterpushed_review_response(
    ...
    if not review.downgrade_to_moderate7 and review.final_bucket != "IMPORTANT retained":
        raise ValueError(
...
def _parse_actionable_score_response(
    ...
    parsed = KernelSecondOpinionModel(
        ...
        real_uaf=real_uaf,
        host_admin_required=host_admin,
        namespace_scoped_privilege=namespace_priv,
        important_candidate=important_candidate,
        manual_review=manual_review,
        auto_downgrade_allowed=auto_downgrade,
...
# Inserted by A.Larkin (on 9 Sept 2026), dirty hack:
    ...
    async def _experimental_kernel_second_opinion(
      ...
      prompt_dump_path = Path(
        f"/tmp/aegis_actionable_score_input_{dump_id}.txt"
      )
    ...
    async def _experimental_kernel_kpanic_review(
        ...
        primary_impact = (
            getattr(result.output, "_original_llm_impact", None)
            or result.output.impact
        )
        primary_score = (
        ...
        try:
            prompt_dump_path.write_text(prompt, encoding="utf-8")
            logger.info(
                "%s: KPANIC_REVIEW exact input dumped to %s "
                "(chars=%d bytes=%d)",
                call_str,
                prompt_dump_path,
                len(prompt),
    ...
    async def _experimental_kernel_afterpushed_review(
        ...
        if isinstance(classifier_result, dict):
            patch_summaries = classifier_result.get("patch_summaries") or []
        if isinstance(patch_summaries, list):
            patch_context = "\n\n--- PATCH ---\n\n".join(str(x) for x in patch_summaries)
        else:
            patch_context = str(patch_summaries)
        ...
        prompt = (
            ...
            + f"Operational KPANIC marker: "
            + ("YES" if getattr(result.output, "_kernel_kpanic_marked", False) else "NO")
            + "\n"
            + f"lowered: "
            + ("YES" if getattr(result.output, "_kernel_lowered", False) else "NO")
            + "\n"
            + f"dec_cnt: {getattr(result.output, '_kernel_decrease_count', 0)}\n"
            + f"pushed_to_high: YES\n\n"
            + consistency_rule
        )
...
# end of "Inserted by A.Larkin, dirty hack"
    ...
    async def exec(self, cve_id: CVEID, static_context: Any = None):
        ...
        if not is_kernel:
            # ----------------------------------------------------------
            # A.Larkin NN+LLM parity POC.
            # BEFORE: only classifier_disagreement_rationale was hidden.
            # AFTER: hide all new kernel-only Perl-parity fields too.
            # WHY: they must not perturb non-kernel SuggestImpact prompts.
            # ----------------------------------------------------------
            ...
            #
            # BEFORE:
            #   The kernel-only fields had defaults, so Gemini could omit them.
            #   Pydantic then silently supplied UNKNOWN/False and the
            #   Perl-compatible rules received no explicit primary-hints signal.
            #
            # AFTER:
            #   Require the three kernel-only parity fields in the structured
            #   schema used for the PRIMARY Gemini call.
            #
            # PERL equivalents:
...
# end of "Inserted by A.Larkin, dirty hack"
        ...
        #
        # BEFORE:
        #   Final export UNIONed the literal classifier kernel_panic flag with
        #   the mutable Perl-style kpanic_marked state. H9/H10/AS6 could remove
        #   KPANIC, but this block then re-added it from active_features.
        #
        # PERL SOURCE:
        #   $flags{$cve} =~ s/ KPANIC//g;
        #
        # AFTER:
        #   If reconcile_kernel() produced a final marker state, export ONLY
        #   that final state. The literal classifier panic is an INPUT/seed to
        #   the state machine, not an unconditional final label.
        #
        #   If no final reconciliation state exists (None), use the old literal
        #   classifier mapping as a compatibility fallback.
        # ------------------------------------------------------------
        if classifier_result and isinstance(classifier_result, dict):
            final_kpanic = getattr(
                result.output,
                "_kernel_kpanic_marked",
                None,
            )
