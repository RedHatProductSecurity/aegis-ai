...
The tool returns **only** the informative signals (active patch-feature
flags and per-class severity probabilities).  It deliberately omits the
predicted impact label and any CVSS vector to avoid anchoring the LLM on
the classifier's output; the model is expected to weigh the signals and
produce its own CVSS / impact assessment.
"""
...
class KernelImpactToolResponse(BaseToolOutput):
    ...
    feature flags (e.g. ``uaf``, ``networking``, ``kernel_panic``) and
    the XGBoost severity class probabilities.  No CVSS vector or single
    impact label is included to avoid anchoring the LLM.
    """
