...
def train_model():
    ...
    feature_columns = [col for col in df.columns if col not in ['cve_id', 'severity_label', 'severity_numeric']]
    ...
    if len(df) < initial_count:
        logger.info(f"Removed {initial_count - len(df)} rows with missing severity_numeric values")
