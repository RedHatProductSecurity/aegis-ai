...
3. **Feature Extraction**: Analyzes patch content and commit messages to extract 48 binary features:
...
**NOTE:** The raw CVE data is typically imbalanced across severity levels (more LOW/MODERATE than IMPORTANT CVEs). This training dataset uses SMOTE balancing (other techniques are being explored) to ensure the model properly classifies all severity levels, especially the IMPORTANT category.
...
[`sample_data/sample_data.csv`](sample_data/sample_data.csv) contains 5 example CVEs showing the expected data format after feature engineering. For production training, update the `data_path` in the script to point to your full engineered dataset.
