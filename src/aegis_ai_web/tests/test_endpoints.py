...
def test_component_intelligence_endpoint_uses_component_name_context():
    ...
    assert data["components"] == ["curl", "libcurl"]
    assert "popularity_score" in data


def test_component_intelligence_endpoint_accepts_title_and_description():
    """Component analysis with title and description returns 200 and includes components."""
    mock_output = ComponentIntelligenceModel(
        component_name="openssl",
        components=["openssl", "libssl"],
        component_latest_version="3.2.0",
        component_purl="pkg:generic/openssl@3.2.0",
        website_url="https://openssl.org",
        ...
        security_information="No critical issues.",
        further_learning="See docs.",
        explanation="Test mock from title+description.",
        confidence=0.9,
        tools_used=[],
        disclaimer="This response was generated by Aegis AI (https://github.com/RedHatProductSecurity/aegis-ai) using generative AI for informational purposes. All findings should be validated by a human expert.",
    )
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["component_name"] == "openssl"
    assert data["components"] == ["openssl", "libssl"]


def test_component_intelligence_endpoint_422_when_invalid_input():
    """Component analysis with neither component_name nor (title+description) returns 422."""
    response = client.get(
        "/api/v1/analysis/component",
        params={"feature": "component-intelligence"},
    )
