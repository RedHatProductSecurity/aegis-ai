...
def _load_expected_flags() -> dict[str, set[str]]:
    """Load expected XGBoost feature flags from al-kernel-cves.csv (column 7)."""
    ...
    with open(CSV_PATH, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if cve_id in expected:
                continue
            raw_flags = row[6].strip() if len(row) > 6 else ""
            expected[cve_id] = {f.lower() for f in raw_flags.split() if f}
    return expected


def _feature_diff(actual: set[str], expected: set[str]) -> dict | None:
    """Return a diff dict or None when both sets are identical."""
...
def audit(results_path: Path) -> dict:
    ...
    expected_flags = _load_expected_flags()
    ...
    for case in cases:
        ...
        predicted = case["predicted_impact"]
        cvss_score = float(case["predicted_cvss3_score"])
        ...
        feat_diff = _feature_diff(clf_features, expected_flags.get(cve_id, set()))
