...
class TestLookupBinaryRPMs:
    ...
    @patch("aegis_ai.toolsets.tools.build_system._resolve_build_from_stream")
    @patch("aegis_ai.toolsets.tools.build_system._create_brew_session")
    def test_deptopia_malformed_json(self, mock_session, mock_resolve):
        mock_session.return_value = MagicMock()
        mock_resolve.side_effect = json.JSONDecodeError("Expecting value", "", 0)
    ...
    @patch("aegis_ai.toolsets.tools.build_system._resolve_build_from_stream")
    @patch("aegis_ai.toolsets.tools.build_system._create_brew_session")
    def test_deptopia_missing_key(self, mock_session, mock_resolve):
        mock_session.return_value = MagicMock()
        mock_resolve.side_effect = KeyError("build_nvr")
